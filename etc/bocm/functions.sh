                        ...
                        "raid1")
                            ...
                            if lvm lvs --noheadings -o lv_name,devices -S vg_name=${_vgname},lv_name=${_lvname}|grep -q ${_devDisk}${part_number}; then
                                ...
                                until [ $syncP = "100" ]; do
                                    ...
                                    printf " Waiting for ${_lvname} mirror syncing: ${syncP}%%"
                                    sleep 1
                                done
                                printf "\ndone\n"
                            else
...
umountAll() {
    ...
    if [ "x${_volFile}" != 'x' ] && [ -f ${_volFile} ]; then
        ...
        log_begin_msg "Unmounting all volumes"
...
bocm_bottom() {
    ...
    # Skasowanie pozostałości środowiska przygotowania szablosnu docker
    if [ -f .dockerenv ]; then
        rm -rf .dockerenv
    fi
    cd /
    ...
    # Wyłaczone poniewaz pochodzi ze starej wersji, gdy system docelowy nie byl restartowany.
    ...
    # Zachowanie pliku logow w systemie docelowym
    mv ${_logfile} ${rootmnt}/var/log/bocm_install.log
